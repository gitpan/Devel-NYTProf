...
=head1 SYNOPSIS
	...
	# produces a report in ./nytprof.out that can be procesed into HTML with
	nytprofhtml

	# or into comma seperated files with
	nytprofcsv
...
=item *
nytprofhtml - creates a very cool and rather elaborate HTML report (including statistics, source code and color highlighting based on speed).
...
=item -
allowfork - Enables fork detection and file locking and disables output buffering.  This will have a severe effect on performance, so use only with 
code that can fork. You B<MUST> use this with code that forks! [default: off]

=item -
useclocktime - Uses real wall clock time instead of CPU time.  With this setting, the profiler will measure time in units of actual microseconds.  The
problem with this is that it includes time that your program was 'running' but
not actually executing in the CPU (maybe it was waiting for its turn to run in
the cpu, or maybe it was suspended).  If you don't know anything about process
scheduling, then don't worry about this setting. [default: off]

=item -
use_stdout - Tells the profiler to write output to STDOUT. [default: 
./nytprof.out]
...
We'll see how the Perl Testers fare...
